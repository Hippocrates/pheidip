...
/**
 * This class is a combo box that allows you to select a date either by typing
 * one in (if the combo box is editable) or through a JCalendar pop-up.
 * Normally, a combo box expects to use a a popup that contains a list. We've
 * perverted this a bit; while you can select a variety of dates, the list
 * always has one item which changes to match the selected date.
 * <p>
 * You may specify a font for each of the elements that make up the pop-up
 * calendar. If you do not specify a font (or specify a null font), a reasonable
 * default will be generated based on the current Look &amp; Feel.
 * <p>
 * The JCalendarCombo uses a non-mutable ComboBoModel. This means that the
 * following methods will generate an exception:
 * <ul>
 * <li>addItem()
 * <li>insertItemAt()
 * <li>removeItem()
 ...
 * <li>removeAllItems()
 * </ul>
 * You would be ill-advised to to call setModel() with your own ComboBoxModel
 * unless you use some variant of a JCalendarCombo.CalendarComboBoxModel class.
 * <p>
 * The combo box listeners work pretty much as for normal combo boxes. The item
 * changed event reports the date "unselected" (the previous date) and
 * "selected" (the new date set), but as Strings. It's probably better to add a
 * DateListener and ignore the ItemListener and ActionListener.
 * <p>
 * The following key bindings should work (some may depend on the selected
 * Look-and-Feel):
 * <ul>
 ...
 * <li>Return - Accept the selected date and hide the calendar pop-up (if
 * visible).
 * </ul>
 * <p>
 * This component tries to adapt it's Look-and-Feel to match the current
 * Look-and-Feel. But it can only do so for Look-and-Feels's that it knows
 * about. To adapt it for new Look-and-Feels, create a sub-class:
 * 
 * <pre>
 * <code>
 ...
 * </pre>
 * 
 * If it can't figure out an appropriate Look-and-Feel, it uses the Metal
 * Look-and-Feel.
 * 
...
public class JCalendarCombo extends JComboBox
{
  ...
  private static InputMap comboBoxInputMap = new InputMap();
  private static ActionMap comboBoxActionMap = new ActionMap();

  private static Action setNullDateTop = new AbstractAction("setNullDateTop")
  {
    public void actionPerformed(ActionEvent e)
    {
      JCalendarCombo combo = (JCalendarCombo)e.getSource();
      combo.calendarPanel.setDate(null);
      combo.hideCalendar();
    }
  };
  
  private static Action openPopup = new AbstractAction("openPopup")
  {
    public void actionPerformed(ActionEvent e)
    {
      JCalendarCombo combo = (JCalendarCombo)e.getSource();
  ...
  static
  {
    ...
    comboBoxInputMap.put(KeyStroke.getKeyStroke("DELETE"), "setNullDateTop");
    comboBoxInputMap.put(KeyStroke.getKeyStroke("DOWN"), "openPopup");
    comboBoxInputMap.put(KeyStroke.getKeyStroke("SPACE"), "openPopup");
    comboBoxInputMap.put(KeyStroke.getKeyStroke("ENTER"), "openPopup");
    
    comboBoxActionMap.put("setNullDateTop", setNullDateTop);
    comboBoxActionMap.put("openPopup", openPopup);
  }
   ...
   * Display the date but not the time. Don't display today's date.
   */
  ...
  /**
   * Create an instance of JCalendarCombo using the default calendar and locale.
   * Display a calendar and a time spinner as requested (to display both use
   ...
   *          True if today's date should be displayed.
   */
  ...
  public JCalendarCombo(Calendar calendar, Locale locale,
      int selectedComponents, boolean isTodayDisplayed)
  {
  ...
  public JCalendarCombo(Calendar calendar, Locale locale,
      int selectedComponents, boolean isTodayDisplayed, String timePattern)
  {
    super();
    calendarPanel = new JCalendar(calendar, locale, selectedComponents,
        isTodayDisplayed, timePattern);
    this.locale = locale;
    ...
    parseFormat[0] = DateFormat.getDateTimeInstance(DateFormat.SHORT,
        DateFormat.SHORT, locale);
    parseFormat[1] = DateFormat.getDateTimeInstance(DateFormat.MEDIUM,
        DateFormat.MEDIUM, locale);
    parseFormat[2] = DateFormat.getDateTimeInstance(DateFormat.LONG,
        DateFormat.LONG, locale);
    parseFormat[3] = DateFormat.getDateTimeInstance(DateFormat.FULL,
    ...
    {
      setDateFormat(DateFormat.getDateTimeInstance(DateFormat.FULL,
          DateFormat.FULL, locale));
    }
  ...
  /**
   * Get the date currently displayed by the calendar panel. If null, then no
   * date was selected.
   * 
  ...
  /**
   * Get the date format used to display the selected date in the combo box's
   * text field.
   * 
   * @return The date format used to display the selected date in the combo
   *         box's text field.
   */
  ...
  /**
   * Set the date format used to display the selected date in the combo box's
   * text field. Nulls are not allowed.
   * 
   * @param dateFormat
   *          The date format used to display the selected date in the combo
   *          box's text field.
   ...
   * argument. The object should be a String representation of a date.
   * <p>
   ...
   * when this method is called (assuming the date actually changed).
   * 
   * @param anObject
  ...
  /**
   * This method is ignored. You cannot change the KeySelectionManager for
   * JCalendarCombo.
   * 
   * @param aManager
   *          The new key selection manager.
   */
  ...
  /**
   * Resets the UI property to a value from the current look and feel. Read the
   * class documentation for instructions on how to override this to make the
   * JCalendarCombo support a new Look-and-Feel.
   */
  ...
  /**
   * Given a date in String form, convert it to a Date object. If no conversion
   * is possible, return null. This method tries to parse the string using
   * DateFormat and SHORT, MEDIUM, LONG and FULL forms. If none of these work, a
  ...
  /**
   * Set up the calendar combo box layout and components. These are not date
   * specific.
   */
  ...
  private void createCalendarComponents()
  {
    ...
    this.setInputMap(WHEN_FOCUSED, comboBoxInputMap);
    this.setActionMap(comboBoxActionMap);
  ...
  protected class CalendarComboBoxModel implements ComboBoxModel
  {
    ...
    public void setSelectedItem(Object anItem)
    {
      ...
      Date date = null;
      if (anItem != null)
        date = stringToDate(anItem.toString());
      ...
      if (date == null && calDate != null || date != null
          && !date.equals(calDate))
      {
      ...
      if (date == null && editorDate != null || date != null
          && !date.equals(editorDate))
      {
      ...
      JCalendarCombo.this.firePropertyChange("date", this.cachedDate, date);
  ...
  private class CalendarKeySelectionManager implements
