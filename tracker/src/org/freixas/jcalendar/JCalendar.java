...
/**
 * This class displays a panel through which a user can select a date and/or
 * time. A time-only selection can be used to select a duration, as long as the
 * duration is no longer than 23 hours, 59 minutes and 59 seconds.
 * <p>
 * The date is selected using a calendar display. The time is selected using a
 * date spinner.
 * <p>
 * In reality, both date and time are part of the same Date returned by
 * getDate() or getCalendar(). If you are selecting only the date, you should
 * ignore the time portion of the Date or Calendar. If you are selecting only
 * the time, ignore the date portion.
 * <p>
 * You can set the pattern used to display the time in the date spinner. The
 * pattern is the same as used by SimpleDateFormat. The default format displays
 * hours, minutes and seconds in a locale-specific way (some locales use AM/PM,
 * some use a 24-hour clock). If you want to get a time duration, you will want
 * to use a pattern such as "HH:mm:ss" to eliminate the possibility of an AM/PM
 * field appearing. You can also use setTimePattern() to reduce the precision of
 * the time obtained (e.g. "HH:mm").
 * <p>
 * You may specify a font for each of the elements that make up the calendar. If
 * you do not specify a font (or specify a null font), a reasonable default will
 * be generated based on the current Look &amp; Feel.
 * <p>
 ...
 * </ul>
 * This is in addition to using Tab and Enter to move through and select the
 * buttons.
 * <p>
 * The time field is divided into hour, minute, second and AM/PM portions. You
 * can select any portion and use the spinner arrows on the right side of the
 * field to increment or decrement that portion. However, the entire time is
 * being incremented or decremented, so that incrementing 1:59:59 by one second
 * will generate 2:00:00.
 * <p>
 * Due to a design limitation in JFormatedTextField, incrementing 24:59:59 will
 * <b>not</b> increment the day. A value in a JFormatedTextField (which is what
 * the time field is), only calculates a date from the fields displayed. Since
 * usually we display a HH:mm:ss pattern, the JFormattedTextField will set the
 * date to a default value, not influenced by the date in the calendar
 * <p>
 * It is possible to pass in a time pattern that displays more than just the
 * time -- this is not advisable since the date portion displayed in the
 * JFormatedTextField will be ignored by the JCalendar component.
 * <p>
 * When the time field has focus, the up/down arrow keys increment or decrement
 * the currently selected time portion, just like the spinner keys. The left and
 * right arrow keys can be used to move to the next or previous portion.
 * 
 * @see Calendar
 * @see Date
 * @see DateFormat
...
public class JCalendar extends JPanel
{
  ...
  private static ResourceBundle bundle = ResourceBundle
      .getBundle("org.freixas.jcalendar.Bundle");
   ...
   * Display the date but not the time. Don't display today's date at the bottom
   * of the panel.
   */
  ...
  /**
   * Create an instance of JCalendar using the default calendar and locale.
   * Display a calendar and/or a time spinner as requested (to display both use
   * DISPLAY_DATE | DISPLAY_TIME). Display today's date if requested. Use the
   * defult pattern to display the time in the time spinner field (if there is
   * one).
   * 
   * @param selectedComponents
   *          Use DISPLAY_DATE, DISPLAY_TIME or (DISPLAY_DATE | DISPLAY_TIME).
   * @param isTodayDisplayed
   *          True if today's date should be displayed at the bottom of the
   *          panel.
   */

  public JCalendar(int selectedComponents, boolean isTodayDisplayed)
  {
    this(Calendar.getInstance(), Locale.getDefault(), selectedComponents,
        isTodayDisplayed, null);
  }
   ...
   * Display a calendar and/or a time spinner as requested (to display both use
   * DISPLAY_DATE | DISPLAY_TIME). Display today's date if requested. Use the
   * default pattern to display the time in the time spinner field (if there is
   * one).
   * 
   * @param calendar
   *          The calendar to use.
  ...
  public JCalendar(Calendar calendar, Locale locale, int selectedComponents,
   ...
   * Create an instance of JCalendar using the given calendar and locale.
   * Display a calendar and/or a time spinner as requested (to display both use
   * DISPLAY_DATE | DISPLAY_TIME). Display today's date if requested. Set the
   * pattern used to display the time in the time spinner field (if there is
   * one). If null, use the default MEDIUM format for the given locale. Patterns
   ...
   *          panel.
   * @param timePattern
   *          The pattern used to display the time in the time spinner field.
   * @see DateFormat
   * @see SimpleDateFormat
   */

  public JCalendar(Calendar calendar, Locale locale, int selectedComponents,
      boolean isTodayDisplayed, String timePattern)
  {
  ...
  /**
   * Add a date listener. This listener will receive events each time the
   * selected date changes.
   * 
   * @param listener
   *          The date listener to add.
   */
  ...
  /**
   * Set whether a null date is allowed. A null date means that no date is
   * selected. The user can select a null date by pressing DELETE anywhere
   * within the calendar.
   * <p>
   * If nulls are not allowed, a setDate(null) will be ignored without error.
   * The DELETE key will do nothing.
   * <p>
   * If you switch from allowing nulls to not allowing nulls and the current
   * date is null, it will remain null until a date is selected.
   * <p>
  ...
  /**
   * Get the date currently displayed by the calendar panel. If no date is
   ...
   * DateEvent. The date may be null. If isNullAllowed() is true, then all dates
   * will be unselected. If isNullAllowed() is false, a null date is ignored.
   * 
   * @param date
   *          The date to set.
   */
  ...
  public void setDate(Date date)
  {
    ...
    else
    {
      ...
      if ((((selectedComponents & DISPLAY_DATE) > 0) && oldDay != selectedDay
          || oldMonth != selectedMonth || oldYear != selectedYear)
          ||

          (((selectedComponents & DISPLAY_TIME) > 0) && oldHour != selectedHour
              || oldMinute != selectedMinute || oldSecond != selectedSecond))
      {
    ...
    firePropertyChange("date", this.cachedDate, this.getDate());
    this.cachedDate = this.getDate();
  }

  /**
   * Reset the displayed date without changing the selected date. No event
   * occurs. A null date will reset to today's date. The display date simply
   * selects the calendar page (month/year) to display.
   * 
   * @param date
   *          The date to display.
   */
   ...
   * This is null if the time is not displayed.
   * 
   * @return The pattern used to display the time in the time selection spinner.
   */
  ...
  /**
   * Get a copy of the calendar used by this JCalendar. This calendar will be
   * set to the currently selected date, so it is an alternative to the
   * getDate() method. If no date is selected (getDate() returns null), the
   * calendar's selected date is 9/9/9999 and should not be used.
   * 
  ...
  /**
   * Return the components being displayed: <code>
   *   (getSelectedComponents() & DISPLAY_DATE) > 0
   * </code> means that the date calendar is being displayed. <code>
   *  (getSelectedComponents() & DISPLAY_TIME) > 0
   * </code> menas that the time spinner field is being displayed.
   * 
   ...
   * 
   * @return True if today's date is displayed at the bottom of the calendar.
   */
  ...
  /**
   * If the font is set to null, then the title font (for the Month Year title)
   * will default to the L&amp;F's Label default font.
   * <p>
  ...
  /**
   * If the font is set to null, then the day-of-week font (Mon, Tue, etc.) will
   * default to 9/11th's of the L&amp;F's Label default font.
   * <p>
  ...
  {
    if (font == null && dayOfWeekFont != null || !font.equals(dayOfWeekFont))
    {
  ...
  /**
   * If the font is set to null, then the day font will default to 9/11th's of
   * the L&amp;F's Button default font.
   * <p>
  ...
  /**
   * If the font is set to null, then the time spinner font will default to the
   * L&amp;F's Spinner default font.
   * <p>
  ...
  {
    if (font == null && timeFont != null || !font.equals(timeFont))
    {
  ...
  /**
   * If the font is set to null, then the font used to display today's date as
   * text will default to the L&amp;F's Label default font.
   * <p>
  ...
  protected String paramString()
  {
    ...
    {
      curDate = DateFormat.getDateInstance(DateFormat.FULL, locale).format(
          getDate());
    }
    else if ((selectedComponents & DISPLAY_TIME) == DISPLAY_TIME)
    {
      curDate = DateFormat.getTimeInstance(DateFormat.FULL, locale).format(
          getDate());
    }
    else
    {
      curDate = DateFormat.getDateTimeInstance(DateFormat.FULL,
  ...
  /**
   * Get the JCalendarCombo component which uses this JCalendar in its combo
   * popup.
   * 
   * @return The JCalendarCombo component which uses this JCalendar in its combo
   *         popup.
   * @see JCalendarCombo
   */
  ...
  /**
   * Set the JCalendarCombo component which uses this JCalendar in its combo
   * popup.
   * 
   * @param comboCalendar
   *          The JCalendarCombo component which uses this JCalendar in its
   *          combo popup.
   * @see JCalendarCombo
   */
  ...
  /**
   * Set up the calendar panel with the basic layout and components. These are
   * not date specific.
   */
  ...
  private void createCalendarComponents()
  {
    ...
    if ((selectedComponents & DISPLAY_DATE) > 0)
    {
      ...
      yearDecrButton = new JButton(new ButtonAction("YearDecrButton",
          "YearDecrButtonMnemonic", "YearDecrButtonAccelerator",
          "YearDecrButtonImage", "YearDecrButtonShort", "YearDecrButtonLong",
          YEAR_DECR_BUTTON));
      monthDecrButton = new JButton(new ButtonAction("MonthDecrButton",
          "MonthDecrButtonMnemonic", "MonthDecrButtonAccelerator",
          "MonthDecrButtonImage", "MonthDecrButtonShort",
          "MonthDecrButtonLong", MONTH_DECR_BUTTON));
      ...
      monthIncrButton = new JButton(new ButtonAction("MonthIncrButton",
          "MonthIncrButtonMnemonic", "MonthIncrButtonAccelerator",
          "MonthIncrButtonImage", "MonthIncrButtonShort",
          "MonthIncrButtonLong", MONTH_INCR_BUTTON));
      yearIncrButton = new JButton(new ButtonAction("YearIncrButton",
          "YearIncrButtonMnemonic", "YearIncrButtonAccelerator",
          "YearIncrButtonImage", "YearIncrButtonShort", "YearIncrButtonLong",
          YEAR_INCR_BUTTON));
  ...
  private void updateCalendarComponents()
  {
    if ((selectedComponents & DISPLAY_DATE) > 0)
    {
...

      {
        Object[] args =
        { month, year };
        monthYearLabel.setText(MessageFormat.format(
      ...
      if (!month.equals(lastMonth) || !year.equals(lastYear))
      {
        ...
        for (int day = 1; day <= maxDays; day++)
        {
          ...
          if (day == selectedDay && displayMonth == selectedMonth
  ...
  private class ButtonAction extends AbstractAction
  {
    ...
    ButtonAction(String name, String mnemonic, String accelerator,
        ...
        int actionId)
    {
      ...
      if (mnemonic != null)
      {
        ...
        {
          putValue(Action.MNEMONIC_KEY, new Integer(bundle.getString(mnemonic)
              .charAt(0)));
        }
      }
  ...
  private class DayListener implements ItemListener
  {
    ...
    public void itemStateChanged(ItemEvent e)
    {
      if (e.getStateChange() == ItemEvent.SELECTED)
      {
        ...
        if (oldDay != selectedDay || oldMonth != selectedMonth
